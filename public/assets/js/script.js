...
(function($) {
    ...
    $('.subscribe-now').click(function(e) {
        e.preventDefault();
        $.get('/subscribe/' + $('input[name="email-subscribe"]').val(), function(res) {
            if (res == 'no') {
                $('.request-msg').html('<div class="alert alert-danger">البريد الالكتروني مسجل مسبقا</div>');
            }
            if (res == 'done') {
                $('.request-msg').html('<div class="alert alert-success">تم التسجيل في النشره البريديه بنجاح</div>');
            }
        })
    })
    $('.slide-1 ').slick({
    ...
    $('.collection-wrapper .qty-box .quantity-right-plus').on('click', function() {
        ...
        var max = parseInt($qty.prop('max'));
        var currentVal = parseInt($qty.val(), 10);
        if (!isNaN(currentVal) && currentVal < max) {
            $qty.val(currentVal + 1);
            $('.product-right_price').text(($qty.val() * Number($('.product-right_price').attr('value'))))
        }
    });
    ...
    /*=====================
     16.  Add to wishlist
     ==========================*/
    $(document).on('click', '.product-box a .ti-heart , .product-box a .fa-heart, .showpage .wishlist-btn', function(e) {

        if ($(this).hasClass('cartActive')) {
            $(this).removeClass("cartActive");
            $(this).parent('.wishlist-btn').removeClass("cartActive");
            removeFromWishlist(e.target.getAttribute('productId'))
            var msg = 'Item Successfully removed from wishlist';
        } else {
            $(this).addClass("cartActive");
            $(this).parent('.wishlist-btn').addClass("cartActive");
    ...
    $('body').on('click', function(e) {
        if (e.target.parentElement.classList.contains('Add-to-Wishlist')) {
            console.log('i.heart-heart')
            $.notify({
                icon: 'fa fa-check',
                title: 'Success!',
    ...
    $('#wishlist_side ul.cart_product').click(function(e) {
        // console.log('productId >>>', e.target.getAttribute('productId') )
        // console.log('ul.cart_product >>>', e.target )
        if (e.target.hasAttribute('productId')) {
            if (e.target.parentElement.classList.contains('close-circle')) {
    ...
    $('#header_search_form_submit').on('click', function() {
            ...
            $(".add_to_cart #cartlist_items").append(`
                <li>
                    <div class="media">
                        ...
                        <img alt="" class="mr-3" src="${val.thumbnail}">
                        </a>
                        ...
                        <h4>
                            <span> $ ${val.price} : x ${val.quantity} </span>
                        </h4>
                        </div>
                    </div>
                    <div class="close-circle">
                            <i class="ti-trash" productId="${val.id}" aria-hidden="true"></i>
                    </div>
                </li>
            `);
        });
...
function addAllToCart(aa) {

    var addallto = function(id, attr, qty) {
        console.log(', [color,attribut] , qty', id, attr, qty);
        let attrbuite = attr === ',' ? '' : attr
            // $.get(`/cart/store?id=${id}&attribute=${attrbuite}&quantityyyyyyyyyyyy=${qty}` , function(response) {
            //     console.log("success>in>blade>response>",response)
            //     document.querySelector(".layout-header2 .cart-totalPrice").textContent = response.totalPrice
    ...
    $.get('/wishlist/get', function(response) {
        // alert("success");
        console.log("wishlist_side", response)

        let length = response.wishlistProducts.length

        let i = 0
            // aa.addToCart(response.wishlistProducts[0].id, '' , 1)
        while (i < length) {
            addallto(response.wishlistProducts[i].id, '', 1)
            i++
        }
...
function openWishlist() {
    ...
    $.get('/wishlist/get', function(response) {
        // alert("success");
        console.log("wishlist_side", response)
        let ids = response.wishlistProducts.reduce((acc, next) => { return acc += "," + next.id }, '')
        console.log("idsidsidsidsidsids ids", ids)
        $.each(response.wishlistProducts, function(i, val) {
            $("#wishlist_side #wishlist_items").append(`
                <li id="close-item">
                    <div class="media">
                        <a href="javascript:void(0)"><img alt="" class="mr-3" src="
                        ${val.thumbnail}
                        "></a>
                        <div class="media-body">
                            <a href="javascript:void(0)"><h4>${val.name}</h4></a>
        ...
        $("#wishlist_side #wishlist_total").append(`
                <li>
                    <div class="buttons">
                        <a href="javascript:void(0)" class="btn btn-normal btn-block view-cart" onclick="addAllToCart(this)">add all to cart</a>
                    </div>
                </li>
                <li>
                    <div class="buttons">
                        <a href="/wishlist" class="btn btn-normal btn-block  view-cart">view wishlist</a>
                    </div>
                </li>
        `);
...
function closeWishlist() {
    ...
    $("#wishlist_side ul.cart_product").html('')
    $("#wishlist_side ul.cart_total").html('')
}
...
//  .category-header-2
$(function() {
    var stickyOffset = $('.layout-header2').offset().top;

    $(window).scroll(function() {
        var width = (window.innerWidth > 1199) ? true : false;
        if (width) {
            var sticky = $('header'),
...
/*=====================
  23. add and change active class on select color on modal
==========================*/
$(function() {
    $('.product-right > .color-variant > li').click(function() {
        $('.color-variant > li').removeClass('active');
        $(this).addClass('active');
    });
    $('.product-right > .product-description ul > li').click(function() {
        $('.product-description ul > li').removeClass('active');
        $(this).addClass('active');
    });
})

/*=====================
  24. Increase and decrease values for inputs on modal
==========================*/
$(function() {
    $('.modal-content [data-type="plus"]').on('click', function() {
        var $qty = $(this).closest('.input-group').find('[name="quantity"]');
        var currentVal = parseInt($qty.val());
        if (!isNaN(currentVal)) {
            $qty.val(currentVal + 1);
            $('.product-right_price').text(($qty.val() * Number($('.product-right_price').attr('value'))))
    ...
    $('.modal-content [data-type="minus"]').on('click', function() {
        var $qty = $(this).closest('.input-group').find('[name="quantity"]');
        var currentVal = parseInt($qty.val());
        if (!isNaN(currentVal) && currentVal > 1) {
            $qty.val(currentVal - 1);
...
/*=====================
  25. make navbar collasped on hover not on click
==========================*/
$('.category-left .nav-left').hover(function() {
    $('.category-left .nav-desk').stop(true, true).delay(200).fadeIn(500);
}, function() {
    $('.category-left .nav-desk').stop(true, true).delay(200).fadeOut(500);
});

/*=====================
26. add read more
==========================*/
$('#prodect_details_read_Btn').on('click', function() {
    var dots = document.querySelector(".prodect_details_read_dots");
    var moreText = document.querySelector(".prodect_details_read");
    var btnText = document.getElementById("prodect_details_read_Btn");
    // $(this).text('readless')
    if (dots.style.display === "none") {
        dots.style.display = "block";
        btnText.innerHTML = "Read more";
        // moreText.style.height = "70px";
        $(".prodect_details_read").removeClass('prodect_details_read_height')
    } else {
        dots.style.display = "none";
        btnText.innerHTML = "Read less";
        // moreText.style.height = "auto";
        $(".prodect_details_read").addClass('prodect_details_read_height')
    }
});

$('#buy_now_button').on('click', function(e) {
    e.preventDefault();
    var attribute = $('.color-variant .active').attr('type');
    var qty = $('#quantity_input').val();
    var id = $('#product_id').val();
    var route = '/buynow?' + 'id=' + id + '&quantity=' + qty + '&attribute=' + attribute;
